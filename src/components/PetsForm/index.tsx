...
export const PetsForm = () => {
  ...
  const {petsStore: {addPet, isError,}} = useStores();
  ...
  useEffect(()=>{
    petTypesGet().then((data)=>{setPetTypeList(data.data)})
  }, [])
  ...
  return (
    <form onSubmit={formik.handleSubmit} className={styles.form}>
      ...
      <select className={styles.input} required={true} value={petType} onChange={(e:any) => setPetType(e.target.value)}>
        {petTypeList && petTypeList.map((data:any) => (
      ...
      <Button
        ...
        title='Добавить'
        type='submit'
        disabled={false}>Добавить</Button>
    </form>
  )
}
